...
export async function build(opts = {}) {
    const { root: monorepoRoot, packager } = findMonorepoRoot(path.join(process.cwd(), opts.appPath || "."));
    // Initialize options
...
function normalizeOptions(opts, root) {
...
function createRevalidationBundle() {
    ...
    fs.copyFileSync(path.join(appBuildOutputPath, ".next", "prerender-manifest.json"), path.join(outputPath, "prerender-manifest.json"));
}
function createImageOptimizationBundle() {
    ...
    try {
        cp.execSync(`npm install --arch=arm64 --platform=linux --target=18 --libc=glibc --prefix="${nodeOutputPath}" sharp@${sharpVersion}`, {
            stdio: "pipe",
            cwd: appPath,
            env: {
                ...process.env,
...
function createCacheAssets(monorepoRoot, disableDynamoDBCache = false) {
    ...
    removeFiles(outputPath, (file) => file.endsWith(".js") ||
    ...
    traverseFiles(outputPath, () => true, (filepath) => {
        ...
        const newFilePath = ext !== "" ? filepath.replace(ext, ".cache") : `${filepath}.cache`;
        switch (ext) {
            case ".meta":
            case ".html":
            case ".json":
            ...
            if (fileData.headers?.["x-next-cache-tags"]) {
                fileData.headers["x-next-cache-tags"]
                    ...
                    metaFiles.push({
                        ...
                        path: {
                            S: path.posix.join(buildId, path.relative(outputPath, filePath).replace(".meta", "")),
                        },
            ...
            traverseFiles(fetchCachePath, () => true, (filepath) => {
                ...
                fileData?.tags?.forEach((tag) => {
                    metaFiles.push({
                        ...
                        path: {
                            S: path.posix.join(buildId, path.relative(fetchCachePath, filepath)),
                        },
                        revalidatedAt: { N: `${Date.now()}` },
                    });
                });
...
/***************************/
async function createServerBundle(monorepoRoot, streaming = false) {
    ...
    //       "serverless-http" package which is not a dependency in user's
    //       Next.js app.
    let plugins = compareSemver(options.nextVersion, "13.4.13") >= 0
        ? [
            openNextPlugin({
    ...
    if (plugins && plugins.length > 0) {
        logger.debug(`Applying plugins:: [${plugins
            .map(({ name }) => name)
...
function injectMiddlewareGeolocation(outputPath, packagePath) {
    ...
    function inject(middlewarePath) {
        const content = fs.readFileSync(middlewarePath, "utf-8");
        fs.writeFileSync(middlewarePath, content.replace("geo: init.geo || {}", `geo: init.geo || {
...
function addPublicFilesList(outputPath, packagePath) {
    // Get a list of all files in /public
    const { appPublicPath } = options;
    const acc = { files: [] };
    function processDirectory(pathInPublic) {
...
function removeCachedPages(outputPath, packagePath) {
    ...
    [".next/server/pages", ".next/server/app"]
        .map((dir) => path.join(dotNextPath, dir))
        .filter(fs.existsSync)
        .forEach((dir) => removeFiles(dir, (file) => file.endsWith(".json") ||
        file.endsWith(".rsc") ||
        file.endsWith(".meta") ||
        (file.endsWith(".html") &&
...
function addCacheHandler(outputPath, options) {
    esbuildSync({
        external: ["next", "styled-jsx", "react"],
...
/* Helper Functions */
/********************/
function esbuildSync(esbuildOptions) {
...
async function esbuildAsync(esbuildOptions) {
    const { openNextVersion, debug } = options;
...
function getNextVersion(nextPackageJsonPath) {
...
function removeNodeModule(nodeModulesPath, modules) {
